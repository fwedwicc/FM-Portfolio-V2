...
const DraftSidebar = () => {
  ...
  return (
    <aside className='md:p-6 p-4 fixed h-screen top-0 left-0 flex md:items-center items-start z-50'>
      ...
      <button className='md:hidden flex items-center justify-center size-11 rounded-[14px] group transition-smooth bg-neutral-950 border border-[#201e2c]'>
        <TbMenu className='size-5 stroke-[1.3px] text-white' />
      </button>
      {/* For Medium to Large Screens */}
      <div className='md:flex hidden flex-col gap-1 border border-neutral-800/60 bg-neutral-950 shadow-lg shadow-neutral-950/20 p-1 rounded-[18px]'>
        {navItems.map(({ id, icon, label }) => (
          <div className='inline-flex group relative transition-smooth' key={id}>
            <a href={`#${id}`} onClick={(e) => scrollToSection(e, `#${id}`)}
              className={`size-12 p-[1px] rounded-[14px] group transition-smooth ${active === id ? 'bg-gradient-to-b from-violet-200 to-indigo-500 text-white shadow-xl shadow-indigo-400/20' : 'bg-[#201e2c] text-indigo-200/40'
                }`}
            >
              <div className={`w-full h-full grid place-content-center group-hover:bg-[#111017] rounded-[13px] transition-smooth ${active === id ? 'bg-neutral-950' : 'bg-[#111111]'}`}>
                {icon}
              </div>
            </a>
            {/* Tooltip */}
